...
async function loadGoogleFont(
    ...
    const url = `https://fonts.googleapis.com/css2?family=${font}${weight ? ':wght@' + weight : ''
...
export default async function GET({ params }: { params: { id: string } }) {
    ...
    const control = control_conv[core.inst_control as never];

    return new ImageResponse(
        (
            <div tw='flex bg-background-50 w-full h-full p-12 flex-col'>
                    ...
                    <div tw='flex flex-col items-start justify-center h-[150px] py-2 pl-2 w-full max-w-[920px]'>
                        ...
                        <div tw='text-[1.375rem] text-stone-700 font-semibold flex flex-row items-center'>
                            ...
                            >
                                ...
                                Students: {(+enrollment.total_pop).toLocaleString()}
                            </span>
                            <svg
                                xmlns='http://www.w3.org/2000/svg'
                                width='24'
                ...
                {university.admissions.acc_rate && university.admissions.yield_rate && <div tw='flex flex-col text-center items-center justify-center'>
                    <div tw='flex flex-col text-center items-center justify-center'>
                        ...
                        <img
                                ...
                                JSON.stringify({
                                    ...
                                    data: {
                                        datasets: [
                                            {
                                                data: [
                                                    university.admissions.acc_rate * 100,
                                                    100 - university.admissions.acc_rate * 100,
                                                ],
                                                backgroundColor: [
                                                    '#00c950',
                                                    '#ababab',
                            ...
                            alt={`Acceptance Rate: ${Math.round(
                                university.admissions.acc_rate * 1000
                            ) / 10
                                }%`}
                            width={100}
                            height={100}
                    ...
                    <div tw='flex flex-row items-center'>
                        Yield Rate:{' '}
                        {Math.round(university.admissions.yield_rate * 1000) /
                            10}
                        %{' '}
                    </div>
                </div>}
